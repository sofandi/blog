...
name="title" value="{{$book->title}}" required autofocus>
@if ($errors->has('title'))
...
<option value="{{$author->id}}" @if($author->id ==
$book->author_id) selected @endif>{{$author->name}}</option>
@empty
@endforelse
</select>
...
name="year" value="{{$book->year}}" required autofocus>
@if ($errors->has('year'))
<span class="help-block">
